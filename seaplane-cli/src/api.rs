...
/// token
pub fn request_token_jwt(api_key: &str) -> Result<String> {
    Ok(request_token(api_key)?.token)
}
...
/// stable)
pub fn request_token(api_key: &str) -> Result<AccessToken> {
...
impl FormationsReq {
    ...
    pub fn refresh_token(&mut self) -> Result<()> {
        self.token = Some(request_token(&self.api_key)?);
        Ok(())
    }
...
macro_rules! maybe_retry {
    ($this:ident . $fn:ident ( $($arg:expr),* ) ) => {{
        ...
        let res = match req.$fn($( $arg ),*) {
            ...
            {
                $this.token = Some(request_token(&$this.api_key)?);
                Ok(req.$fn($( $arg ,)*)?)
            }
            Err(e) => Err(e),
        };
