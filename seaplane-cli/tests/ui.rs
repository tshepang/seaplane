...
mod tests {
    ...
    fn seaplane_formation_common() {
        ...
        // invalid
        assert!(
            cli!("formation create --affinity way-too-many-hyphens-to-pass-validation").is_err()
        );
        // multiples only with commas or multiple uses
        ...
        // invalid
        assert!(
            cli!("formation create --connection way-too-many-hyphens-to-pass-validation").is_err()
        );
        // multiples only with commas or multiple uses
        ...
        // alias
        assert!(
            cli!("formation create --providers=Aws,Azure,DigitalOcean,Equinix,Gcp,All").is_ok()
        );

        // exlucde provider
        // valid
        assert!(
            cli!("formation create --exclude-provider=Aws,Azure,DigitalOcean,Equinix,Gcp,All")
                .is_ok()
        );
        // invalid
        ...
        // multiples only with commas or multiple uses
        assert!(cli!(
            "formation create --exclude-provider=Aws,Azure --exclude-provider=DigitalOcean"
        )
        .is_ok());
        assert!(cli!(
            "formation create --exclude-provider=Aws --exclude-provider=DigitalOcean,Azure"
        )
        .is_ok());
        assert!(
        ...
        // alias
        assert!(cli!(
            "formation create --exclude-providers=Aws,Azure,DigitalOcean,Equinix,Gcp,All"
        )
        .is_ok());
        ...
        assert!( cli!("formation create --public-endpoint=http:foo/bar=baz:123 --public-endpoint=http:/=que:5432") .is_ok());
    ...
    fn seaplane_formation_create() {
        ...
        assert!(
            cli!("formation create --flight-image foo --flight-arch=amd64,arm64 --flight-arch=amd64,arm64").is_ok()
        );
        assert!(cli!(
            "formation create --flight-image foo --flight-arch=amd64 --flight-arch=amd64"
        )
        .is_ok());
        assert!(cli!(
